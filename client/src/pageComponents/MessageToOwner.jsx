...
const MessageToOwner = () => {
    ...
    useEffect(() => {
        const fetchUserId = async () => {
            try {
                const response = await fetch(`https://flatmate-c9up.onrender.com/api/users/profile/getUserProfile/${selectedUserId}`)
                        ...
                        <>
                            ...
                            <div className="p-4 border-b">
                                <div className="flex items-center gap-3">
                                    <Avatar>
                                        <AvatarFallback>
                                            {selectedUser.userName ? selectedUser.userName.split(' ').map(n => n[0]).join('') : selectedUser.name.split(' ').map(n => n[0]).join('')}
                                        </AvatarFallback>
                                    </Avatar>
                                    <div>
                                        <h3 className="font-semibold">{selectedUser.userName ? selectedUser.userName : selectedUser.name}</h3>
                                    </div>
                                </div>
                            </div>
